...
public class DailyDiaryService {
    ...
    public void writeDailyDiary(Long memberId, DailyDiaryCreateModifyRequestDTO request, LocalDateTime now) {
        // 업로드할 일기방 속한 멤버인지 확인
            ...
            for (Long diaryRoomId : request.getDiaryRooms()) {
                memberDiaryRooms.stream()
                        ...
                        .ifPresent(memberDiaryRoom ->
                                dailyDiaryRepository.save(
                                        DailyDiary.of(
                                                request.getContent(),
                                                memberDiaryRoom.getDiaryRoom(),
                                                request.getFeeling(),
